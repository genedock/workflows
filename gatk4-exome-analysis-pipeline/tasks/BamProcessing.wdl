...
task SortSam {
  ...
  runtime {
    docker: "genedockdx/genomes-in-the-cloud:2.4.3-1564508330"
...
task SortSamSpark {
  input {
    ...
    Int preemptible_tries
    Int compression_level
    String gatk_docker = "genedockdx/gatk:4.0.10.1"
  }
  # SortSam spills to disk a lot more because we are only store 300000 records in RAM now because its faster for our data so it needs
  # more disk space.  Also it spills to disk in an uncompressed format so we need to account for that with a larger multiplier
...
task MarkDuplicatesSpark {
  ...
  runtime {
    docker: "genedockdx/gatknightly:gatkMasterSnapshot44ca2e9e84a"
...
task GenerateSubsettedContaminationResources {
  ...
  runtime {
    ...
    docker: "genedockdx/bedtools:2.27.1"
  }
  output {
...
task CheckContamination {
  ...
  runtime {
    ...
    memory: "7.5 GiB"
    disks: "local-disk " + disk_size + " HDD"
    docker: "genedockdx/verify-bam-id:c1cba76e979904eb69c31520a0d7f5be63c72253-1553018888"
    cpu: 2
  }
  output {
